...
def compute_paired_adversarial_acc(y_true, y_pred, qa_types, image_ids):
    """
    Paired adversarial metric matching calculate_score.py:
    For each (image, category) pair, both the gt and hallu questions must be
    answered correctly to count as correct (acc_with_pair).
    acc_without_pair only requires the gt question to be correct.
    """
    ...
    for img_id, qts in img_qts.items():
        ...
        for qt in qts:
            if qt.startswith("entity_gt_"):
                n = qt[len("entity_gt_"):]
                hallu_key = f"entity_hallu_{n}"
                if hallu_key in qt_set:
                    pairs.append((correct[(img_id, qt)], correct[(img_id, hallu_key)]))

        for qt in qts:
            if qt.startswith("grounding_gt_"):
                n = qt[len("grounding_gt_"):]
                hallu_key = f"grounding_hallu_{n}"
                if hallu_key in qt_set:
                    pairs.append((correct[(img_id, qt)], correct[(img_id, hallu_key)]))
...
def main():
    ...
    all_results["logistic_regression"] = compute_metrics(y_test, clf.predict(H_test_scaled), qa_test, ids_test)
    ...
    with open(csv_file, "w", newline="") as f:
        ...
        writer.writeheader()
        for method, res in all_results.items():
            writer.writerow({"method": method, **{m: f"{res.get(m, float('nan')):.4f}" for m in metric_cols}})
    ...
    # Subplots: one per metric, bars = methods
    ...
    fig.suptitle("Correction Model Comparison", fontsize=13, fontweight="bold")
    fig.tight_layout()
    subplot_file = os.path.join(args.output_dir, "results_subplots.png")
    fig.savefig(subplot_file, dpi=150)
    plt.close(fig)
