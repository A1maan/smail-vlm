"""
Extract CheXagent hidden states at an intermediate transformer layer.
Default target layer: 21 (probing peak for CheXagent).
"""
...
def load_questions(inference_file, image_folder, test_file):
    ...
    for r in data:
        ...
        model_pred = "yes" if ("Yes" in response or response.lower().strip() in ("yes", "yes.")) else "no"
        questions.append({
...
def run_extraction(extractor, questions):
    ...
    for idx, q in enumerate(tqdm(questions, desc="Extracting")):
        if not os.path.exists(q["image_path"]):
            ...
            continue
        try:
            hidden, yes_logit, no_logit = extractor.extract(q["image_path"], q["question"])
...
def main():
    ...
    parser.add_argument("--target-layer", type=int, default=21,
                        help="Which hidden_states[L] to extract (default 21, probing peak)")
