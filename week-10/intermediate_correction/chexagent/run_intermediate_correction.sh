#!/usr/bin/env bash
# CheXagent intermediate correction — full 3-step pipeline
#
# Usage:
#   bash run_intermediate_correction.sh
#   LAYER=18 bash run_intermediate_correction.sh   # sweep a different layer
set -euo pipefail
...
MODE="${MODE:-probe}"
NUM_GPUS="${NUM_GPUS:-4}"
RESULTS_FILE="${RESULTS_FILE:-../response_file/chexagent.json}"
TEST_FILE="${TEST_FILE:-../../../test/test.json}"
IMAGE_FOLDER="${IMAGE_FOLDER:-../../../test}"

CACHE_DIR="chexagent/results/hidden_states_layer${LAYER}"
TRAIN_DIR="chexagent/results/correction_layer${LAYER}"
EVAL_DIR="chexagent/results/forwarded_layer${LAYER}/${MODE}_strength_${STRENGTH}"

echo "============================================================"
echo "CheXagent Intermediate Correction  (layer=${LAYER})"
echo "============================================================"

echo ""
echo "[Step 1] Extracting intermediate hidden states at layer ${LAYER}..."
"$PYTHON_BIN" chexagent/run_extract_batch.py \
...
"$PYTHON_BIN" chexagent/intermediate_correction_chexagent.py \
