"""
Extract MedGemma hidden states at an intermediate transformer layer.
Default target layer: 18 (mid-point; MedGemma best probing layer is the
final layer 26, so pick an earlier layer for the intermediate experiment).
Run with --target-layer to sweep and find the best intermediate layer.
"""
...
def parse_model_pred(response):
    r = response.strip()
    if "Yes" in r or r.lower() in ("yes", "yes."):
        return "yes"
    return "no"
...
class HiddenStateExtractor:
    ...
    def __init__(self, model_name, target_layer):
        ...
        num_lang_layers = len(self.model.model.language_model.layers)
        print(f"Model loaded. Language layers: {num_lang_layers}  "
              f"Yes={self.yes_token_id}, No={self.no_token_id}")
...
def load_questions(inference_file, image_folder, test_file):
    ...
    for r in data:
        ...
        if image_file is None:
            missing += 1
            continue
        model_pred = parse_model_pred(r.get("response", ""))
        questions.append({
            "id": qid, "question": question, "gt_ans": gt_ans,
            "model_pred": model_pred, "is_correct": model_pred == gt_ans,
...
def main():
    ...
    parser.add_argument("--target-layer", type=int, default=18,
                        help="Which hidden_states[L] to extract. MedGemma probing peaks at "
                             "layer 26 (final); use an earlier layer (e.g. 15-20) here.")
    ...
    extractor   = HiddenStateExtractor(args.model_name, args.target_layer)
